...
void manual(void){
    ...
    UART2_SendString("Press A to increase duty cycle or L to decrease duty cycle or 0 to return to mode selection\n");
    ...
    while (1) {
        ...
        if (input == 'A') {
            duty_cycle += 1;
            if (duty_cycle > max_point) {
                duty_cycle = max_point;
            }
            TIM4->CCR2 = duty_cycle;
            sprintf(duty_cycle_str, "%d", duty_cycle);
            UART2_SendString("Duty cycle: ");
            ...
            duty_cycle -= 1;
            if (duty_cycle < startpoint) {
                duty_cycle = startpoint;
            }
            sprintf(duty_cycle_str, "%d", duty_cycle);
            UART2_SendString("Duty cycle: ");
            UART2_SendString(duty_cycle_str);
            ...
            UART2_SendString("Returning to mode selection\n");
            break;
        }
        else {
            UART2_SendString("Invalid key. Press + to increase duty cycle or - to decrease duty cycle or 0 to return to mode selection\n");
        }
    }
